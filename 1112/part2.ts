...
const getOperationResult = (operation: string, oldValue: bigint): bigint => {
...
const prepareInitData = () => {
  monkeysRawData.forEach((monkeyData: string, index: number) => {
    const startingItems: bigint[] = monkeyData.match(STARTING_ITEMS_REGEX)[0].split(', ').map((val: string) => BigInt(val));
    const operation: string = monkeyData.match(OPERATION_REGEX)[0];
    const divisibleBy: bigint = BigInt(monkeyData.match(DIVISIBLE_BY_REGEX)[0]);
...
const megaMod = monkeysArray.reduce((acc: bigint, monkey: IMonkey) => acc * monkey.divisibleBy, BigInt(1))

for (let round = 0; round < NUMBER_OF_ROUNDS; round++) {
  monkeysArray.forEach((monkey: IMonkey) => {
    monkey.items.forEach((itemWorryLevel: bigint) => {
      const worryLevel: bigint = getOperationResult(monkey.operation, itemWorryLevel % megaMod);
      const destinationMonkey = Number(worryLevel) % Number(monkey.divisibleBy) === 0 ? monkey.testTrueResult : monkey.testFalseResult;
      monkeysArray[destinationMonkey].items.push(BigInt(worryLevel));
      monkey.activity++;
    })
    monkey.items = [];
  })
