...
const input = `CjhshBJCSrTTsLwqwqwb
...
rucksacks.forEach((rucksack: string) => {
  ...
  const firstComp = rucksack.slice(0, halfLength);
  const secondComp = rucksack.slice(halfLength, rucksack.length);
  firstComp.split('').some(letter => {
    if (secondComp.includes(letter)) {
      totalPriority += alphabet.indexOf(letter) + 1;
      return true;
    }
  })
  console.log('aa', {
