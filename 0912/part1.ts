...
const checkIfTailShouldMove = (): boolean => {
  const tailPossibleCords: [number, number][] = [
    ...
    [headCords[0] + 1, headCords[1] + 1],
    [headCords[0] - 1, headCords[1]],
    [headCords[0], headCords[1]],
    [headCords[0] + 1, headCords[1]],
    [headCords[0] - 1, headCords[1] - 1],
    [headCords[0], headCords[1] - 1],
    [headCords[0] + 1, headCords[1] - 1],
...
const moveHead = (direction: string) => {
  const prevHeadCords: [number, number] = [...headCords];
  headCords[HEAD_GRID_CHANGES[direction].axis] += HEAD_GRID_CHANGES[direction].change;
  const shouldMoveTail = checkIfTailShouldMove();
  if (shouldMoveTail) {
    moveTail(prevHeadCords);
  }
}

const moveTail = (newCords: [number, number]) => {
  tailCords = [...newCords];
  visitedCells.add(`${tailCords[0]}${tailCords[1]}`);
}
