...
const graphSearch = (minutesLeft: number) => {
  ...
  while (queue.length > 0) {
    ...
    const visitedKey = `${currentValve}:${remainingTime}:${openMask}:${totalGasOut}`;

    if (visited.has(visitedKey)) continue;

    visited.add(visitedKey);

    maxGasOut = Math.max(item.totalGasOut, maxGasOut);
    ...
    for (let i = 0; i < valvesWithFlow.length; i++) {
      ...
      if (isValveOpened(openMask, nextValve) || nextRemainingTime <= 0) {
        continue;
      }
      queue.push({
        currentValve: nextValve,
        openMask: addOpenValveMask(openMask, nextValve),
